...
public class Solution {
    ...
    static LocalDateTime changeZone(LocalDateTime fromTime, ZoneId fromZone, ZoneId toZone) {
        long fromZOffset = fromZone.getRules().getOffset(fromTime).getTotalSeconds();
        long toZOffset = toZone.getRules().getOffset(fromTime).getTotalSeconds();
        return ZonedDateTime.of(fromTime,toZone).toLocalDateTime().plusSeconds(toZOffset - fromZOffset);
    }
}
